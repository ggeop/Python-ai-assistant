...
class ActionControllerTests(unittest.TestCase):
    ...
    def test_tell_me_about(self, mocked_wiki):
        ...
        ActionManager.tell_me_about(voice_transcript=words, action=None, tag='about')
        self.assertEqual(1, mocked_wiki.call_count)
        words = 'about google'
        self.assertRaises(Exception, ActionManager.tell_me_about(voice_transcript=words, action=None, tag='about'))
